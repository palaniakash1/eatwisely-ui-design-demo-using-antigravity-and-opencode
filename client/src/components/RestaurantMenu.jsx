...
export default function RestaurantMenu({ menu }) {
  ...
  const getUniqueCategories = () => {
    ...
    return menu.map(cat => cat.category || cat.name || 'Unknown').filter(Boolean)
  }
                      ...
                      <div className="flex justify-between items-start mb-2">
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            {item.isMeal && (
                              <Badge color="info" size="xs">Meal</Badge>
                            )}
                            {item.dietary?.vegetarian && (
                              <Badge color="success" size="xs">V</Badge>
                            )}
                            {item.dietary?.vegan && (
                              <Badge color="success" size="xs">VG</Badge>
                            )}
                          </div>
                      ...
                      {item.ingredients && item.ingredients.length > 0 && (
                        <div className="mt-2">
                          <p className="text-xs text-gray-500 mb-1">
                            <span className="font-medium">Ingredients:</span> {item.ingredients.map(i => i.name || i).join(', ')}
                          </p>
                          {item.ingredients.some(i => i.allergens?.length > 0) && (
                            <div className="flex flex-wrap gap-1 mt-1">
                              {item.ingredients.flatMap((i, idx) => 
                                (i.allergens || []).map((allergen, aIdx) => (
                                  <Badge key={`${idx}-${aIdx}`} color="warning" size="xs">
                                    {allergen}
                                  </Badge>
                                ))
                              )}
                        ...
                        <div className="mt-2 flex flex-wrap gap-2 text-xs">
                          {item.nutrition.calories && (
                            <span className={`px-2 py-1 rounded ${
                              item.nutrition.calories.level === 'green' ? 'bg-green-100 text-green-800' :
                              item.nutrition.calories.level === 'amber' ? 'bg-yellow-100 text-yellow-800' :
                          ...
                          {item.nutrition.fat && (
                            <span className={`px-2 py-1 rounded ${
                              item.nutrition.fat.level === 'green' ? 'bg-green-100 text-green-800' :
                              item.nutrition.fat.level === 'amber' ? 'bg-yellow-100 text-yellow-800' :
                              'bg-red-100 text-red-800'
                            }`}>
                      ...
                      {item.upsells && item.upsells.length > 0 && (
                        <div className="mt-2 flex flex-wrap gap-2">
                          {item.upsells.map((upsell, uIdx) => (
                            <span key={uIdx} className="text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded">
