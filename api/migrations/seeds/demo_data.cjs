...
const connectDB = async () => {
  try {
    await mongoose.connect(process.env.MONGO_URI || 'mongodb://localhost:27017/mern-restaurant');
...
const createDemoData = async () => {
  try {
    ...
    const restaurants = await Restaurant.insertMany([
      ...
      },
      {
        ...
        address: {
          ...
          postcode: 'W8 7RG',
          country: 'United Kingdom',
          location: { type: 'Point', coordinates: [-0.1937, 51.5010] }
        },
        openingHours: {
    ...
    const genericCategories = await Category.insertMany([
      { name: 'Starters', slug: 'starters', isGeneric: true, order: 1, status: 'published', isActive: true },
      { name: 'Main Courses', slug: 'main-courses', isGeneric: true, order: 2, status: 'published', isActive: true },
      { name: 'Desserts', slug: 'desserts', isGeneric: true, order: 3, status: 'published', isActive: true },
      { name: 'Drinks', slug: 'drinks', isGeneric: true, order: 4, status: 'published', isActive: true }
    ]);
    ...
    const restaurantCategories = await Category.insertMany([
      { name: "Chef's Specials", slug: 'demo-london-grills-chefs-specials', isGeneric: false, restaurantId: restaurants[0]._id, order: 0, status: 'published', isActive: true },
      { name: "Chef's Specials", slug: 'demo-italian-bistro-chefs-specials', isGeneric: false, restaurantId: restaurants[1]._id, order: 0, status: 'published', isActive: true }
    ]);
    console.log(`Created ${restaurantCategories.length} restaurant-specific categories`);
        ...
        items: [
          {
            ...
            description: 'Toasted ciabatta with fresh tomatoes, garlic, basil and olive oil',
        ...
        items: [
          {
            ...
            description: '12oz premium ribeye with truffle fries and peppercorn sauce',
          ...
          },
          {
            ...
            description: 'Beyond Meat patty with vegan cheese, lettuce, tomato and special sauce',
    ...
    restaurantCategories.forEach(c => console.log(`  - ${c.name} (restaurant-specific)`));
    console.log('\nMenus:');
