...
export const placeAutocomplete = async (input, language = 'en') => {
  ...
  try {
    const response = await axios.get(`${PLACES_API_BASE_URL}/autocomplete/json`, {
      params: {
        input: input.trim(),
        types: 'address',
        language,
    ...
    return {
      ...
      data: predictions.map((prediction) => ({
        ...
        structuredFormat: {
          ...
          secondaryText: prediction.structured_formatization?.secondary_text || ''
        },
        matchedSubstrings: prediction.matched_substrings?.map((m) => ({
          offset: m.offset,
          length: m.length
        })) || []
      }))
    ...
    logger.error('places_api.autocomplete_exception', { input, error: error.message });
    throw errorHandler(500, 'Failed to fetch address suggestions');
  }
};
...
export const getPlaceDetails = async (placeId) => {
  ...
  try {
    ...
    const getComponent = (types) => {
      const component = addressComponents.find((c) => c.types.some((t) => types.includes(t)));
    ...
    const address = {
      ...
      areaLocality: getComponent(['sublocality', 'locality', 'neighborhood']) || '',
      city: getComponent(['postal_town', 'locality']) || '',
      countyRegion: getComponent(['administrative_area_level_1', 'administrative_area_level_2']) || '',
    ...
    return {
      ...
      data: {
        ...
        address: {
          ...address,
          addressLine1: address.addressLine1 ? `${address.addressLine1} ${address.addressLine2}`.trim() : address.addressLine2
        },
    ...
    if (error.statusCode) throw error;
    logger.error('places_api.details_exception', { placeId, error: error.message });
...
export const searchPlaces = async (query, location = null, radius = 5000) => {
  ...
  try {
    ...
    if (error.statusCode) throw error;
    logger.error('places_api.search_exception', { query, error: error.message });
    throw errorHandler(500, 'Failed to search places');
  }
};
