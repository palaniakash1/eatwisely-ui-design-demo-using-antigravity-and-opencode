...
const calculateNameSimilarity = (name1, name2) => {
  ...
  return Math.max(tokenScore * 0.6 + prefixScore * 0.4, commonTokens.length / Math.max(tokens1.length, tokens2.length));
};

const extractBusinessType = (name) => {
  const lower = name.toLowerCase();
  const typePatterns = [
    { pattern: /\b(restaurant|cafe|café|bistro|eatery|dining)\b/i, type: 'restaurant' },
    { pattern: /\b(takeaway|takeaway|take away|fast food|kebab|pizza|burger|chippy|chinese|indian|thai|vietnamese|mexican)\b/i, type: 'fast_food' },
    { pattern: /\b(pub|inn|bar|hotel)\b/i, type: 'pub' },
    { pattern: /\b(hotel|resort|lodge|inn)\b/i, type: 'hotel' },
    { pattern: /\b(school|college|university|hospital|care home|nursing home)\b/i, type: 'institutional' },
...
export const findBestMatch = (establishments, targetName, targetPostcode = null) => {
  if (!establishments || establishments.length === 0) {
    return { match: null, score: 0, multipleMatches: false };
  }

  const normalizedTargetPostcode = targetPostcode ? normalizePostcode(targetPostcode) : null;
  ...
  const scored = establishments.map((est) => {
    ...
    if (normalizedTargetPostcode && est.PostCode) {
      ...
      if (estPostcode === normalizedTargetPostcode) {
        postcodeScore = 1;
      } else if (estPostcode.startsWith(normalizedTargetPostcode.split(' ')[0])) {
        postcodeScore = 0.5;
      }
    }

    const typeScore = targetType === extractBusinessType(est.BusinessName) ? 0.15 : 0;
...
export const searchAndMatchEstablishment = async (name, postcode = null) => {
  try {
    ...
    const estArray = Array.isArray(establishments) ? establishments : [establishments];
    ...
    return {
      ...
      multipleOptions: matchResult.multipleMatches ? estArray.slice(0, 5) : null,
