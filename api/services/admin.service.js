...
export const createPrivilegedUser = async ({ actor, body, req }) => {
  return traceAuthOperation(
    ...
    async () => {
      ...
      if (!['admin', 'storeManager', 'superAdmin'].includes(role)) {
      ...
      if (role === 'superAdmin') {
        const existingSuperAdmins = await adminRepository.findUsersCount({ role: 'superAdmin' });
        if (existingSuperAdmins > 0) {
          throw errorHandler(400, 'A superAdmin already exists. Only one superAdmin is allowed.');
        }
      }
...
export const updatePrivilegedUser = async ({
  ...
  return traceAuthOperation(
    ...
    async () => {
      ...
      if (!['admin', 'storeManager', 'superAdmin'].includes(existingUser.role)) {
        throw errorHandler(
          400,
          'Only admin, storeManager, or superAdmin accounts can be updated here'
        );
      }

      const nextRole = body.role || existingUser.role;
      if (!['admin', 'storeManager', 'superAdmin'].includes(nextRole)) {
        throw errorHandler(400, 'Invalid role assignment');
      }

      if (nextRole === 'superAdmin' && existingUser.role !== 'superAdmin') {
        const existingSuperAdmins = await adminRepository.findUsersCount({ role: 'superAdmin' });
        if (existingSuperAdmins > 0) {
          throw errorHandler(400, 'A superAdmin already exists. Only one superAdmin is allowed.');
        }
      ...
      if (body.profilePicture !== undefined) {
        updates.profilePicture =
          body.profilePicture || existingUser.profilePicture;
      }
      ...
      if (Object.prototype.hasOwnProperty.call(body, 'permissions')) {
        ...
        const requestedPermissionCount = Object.values(
          body.permissions || {}
        ).reduce(
          (count, actions) =>
            count + (Array.isArray(actions) ? actions.length : 0),
      ...
      const updatedUser = await adminRepository.updateUser(
        targetUserId,
        updates
      );
      ...
      return {
        ...
        permissionMode: updatedUser.customPermissions
          ? 'custom'
          : 'roleTemplate'
      };
    }
